...
// Récupération des données du produit si elles sont disponibles
$produit = isset($produit) ? $produit : null; // Si le produit est passé depuis le contrôleur
?>
        ...
        <form method="POST" action="/produits/modifierProduit=<?=$produit['id']?>" enctype="multipart/form-data" class="space-y-8">
            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="nom" name="nom" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" 
                            value="<?php echo $produit ? htmlspecialchars($produit['nom']) : ''; ?>" required>
                    </div>
                    <div>
                        <label for="prix" class="block text-sm font-medium text-gray-700">Prix unitaire</label>
                        <input type="number" id="prix" name="prix_unitaire" step="0.01" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" 
                            value="<?php echo $produit ? htmlspecialchars($produit['prix_unitaire']) : ''; ?>" required>
                    </div>
                    <div>
                        <label for="quantite" class="block text-sm font-medium text-gray-700">Quantité</label>
                        <input type="number" id="quantite" name="quantite" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" 
                            value="<?php echo $produit ? htmlspecialchars($produit['quantite']) : ''; ?>" required>
                    </div>
                    <div>
            ...
            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                ...
                <div>
                    ...
                    <input type="text" id="courteDescription" name="courte_description" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" 
                        value="<?php echo $produit ? htmlspecialchars($produit['courte_description']) : ''; ?>" required>
                </div>
                <div>
                    <label for="longueDescription" class="block text-sm font-medium text-gray-700">Longue description</label>
                    <textarea id="longueDescription" name="longue_description" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" rows="3" required>
                        <?php echo $produit ? htmlspecialchars($produit['description']) : ''; ?>
                    </textarea>
                </div>
            </div>
            ...
            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="model" name="model" class="mt-1 p-2 block w-full border-gray-300 rounded-lg" 
                            value="<?php echo $produit ? htmlspecialchars($produit['model']) : ''; ?>" required>
                    </div>
                    <div>
